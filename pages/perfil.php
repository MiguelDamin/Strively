...
<div class="perfil-layout">
    ...
    <!-- MENSAGENS -->
    <div
        style="position: absolute; top: 80px; left: 0; right: 0; max-width: 600px; margin: 0 auto; z-index:100; padding: 0 20px;">
    ...
    <aside class="perfil-sidebar">
        ...
        <p class="ps-cidade-view">📍
            <?= !empty($usuario['cidade']) ? htmlspecialchars($usuario['cidade']) : 'Cidade não informada' ?></p>
    ...
    <main class="perfil-conteudo">
        ...
        <div id="tab-sobre" class="tab-content active">
            ...
            <div class="ps-eventos-header">
                ...
                <select class="ps-filter-select" onchange="window.location.href='?filtro_evento='+this.value">
                    ...
                    <option value="participados" <?= $filtro === 'participados' ? 'selected' : '' ?>>Já participados
                    </option>
                </select>
            </div>
            ...
            <?php if (empty($eventos_perfil)): ?>
                <p
                    style="color: #888; background: #fff; padding: 24px; border-radius: 16px; text-align: center; box-shadow: var(--shadow-md);">
                ...
                <?php foreach ($eventos_perfil as $ev): ?>
                    ...
                    <div class="ps-evento-card">
                        <div class="ps-evento-img" <?= $hasBanner ? 'style="' . $capaArray[0] . '"' : '' ?>>
                            <?php if (!$hasBanner): ?>
                                <svg style="width:50px; height:50px; fill:#ccc;" viewBox="0 0 24 24">
                                    <path
                        ...
                        <div class="ps-evento-info">
                            ...
                            <p class="ps-evento-detalhes">📅 <?= $dt->format('d/m/Y') ?> &nbsp;&nbsp;&nbsp;
                                <?= htmlspecialchars($cidade) ?></p>
        ...
        <div id="tab-treinos" class="tab-content">
            ...
            <?php if (empty($ultimos_treinos)): ?>
                <p
                    style="color: #888; background: #fff; padding: 24px; border-radius: 16px; text-align: center; box-shadow: var(--shadow-md);">
                    Você ainda não tem treinos. Encontre um treinador ou adicione você mesmo!</p>
            <?php else: ?>
    ...
    <div class="modal-content">
        ...
        <form action="/actions/action-perfil.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="upload-wrapper">
                <?php if (!empty($usuario['foto'])): ?>
                    <img src="<?= htmlspecialchars($usuario['foto']) ?>" alt="Preview" class="preview-modal-foto"
                        id="imgPreviewModal" />
                <?php else: ?>
                    <img src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23aaa'%3E%3Cpath d='M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z'/%3E%3C/svg%3E"
                        alt="Preview" class="preview-modal-foto" id="imgPreviewModal" />
                <?php endif; ?>
                <div>
                    <label for="inputFotoModal" class="btn-mudar-foto">Alterar foto</label>
                    <input type="file" id="inputFotoModal" name="foto" style="display:none;" accept="image/*"
                        onchange="atualizarPreviewModal(this)">
                </div>
            </div>
            ...
            <div class="form-grupo">
                ...
                <input type="text" id="tipo_corredor" name="tipo_corredor"
                    value="<?= htmlspecialchars($usuario['tipo_corredor'] ?? '') ?>"
                    placeholder="Ex: Velocista, Maratonista, Triatleta..." />
            </div>
            ...
            <div class="form-grupo">
                ...
                <select id="nivel" name="nivel">
                    ...
                    <option value="Iniciante" <?= ($usuario['nivel'] ?? '') === 'Iniciante' ? 'selected' : '' ?>>Iniciante
                    </option>
                    <option value="Amador" <?= ($usuario['nivel'] ?? '') === 'Amador' ? 'selected' : '' ?>>Amador</option>
                    <option value="Competitivo" <?= ($usuario['nivel'] ?? '') === 'Competitivo' ? 'selected' : '' ?>>
                        Competitivo</option>
            ...
            <div class="form-grupo">
                ...
                <textarea id="bio" name="bio" rows="4" maxlength="300" oninput="updateCharCount(this)"
                    placeholder="Ex: 4xUltramaratonista, 6xMaratonista"><?= htmlspecialchars($usuario['bio'] ?? '') ?></textarea>
