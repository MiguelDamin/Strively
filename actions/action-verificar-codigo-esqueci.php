...
if (empty($codigoDigitado) || $codigoDigitado !== $codigoCorreto) {
    header('Location: /pages/esqueci-senha.php?etapa=codigo&erro=codigo_invalido');
    exit();
}
...
if (time() > $expiracao) {
    ...
    header('Location: /pages/esqueci-senha.php?etapa=codigo&erro=codigo_expirado');
    exit();
}

// Senha validada! Libera a etapa de redefinição
$_SESSION['recuperacao_autorizada'] = true;
header('Location: /pages/esqueci-senha.php?etapa=nova_senha');
exit();
